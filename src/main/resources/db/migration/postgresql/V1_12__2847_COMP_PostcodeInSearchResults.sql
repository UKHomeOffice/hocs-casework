CREATE OR REPLACE VIEW casework.correspondents_json_by_case AS
    SELECT case_uuid, json_build_object('correspondents', jsonb_agg(json_build_object('fullname', fullname, 'postcode', postcode, 'type', "type", 'is_primary', is_primary))) as correspondents
    FROM (
      SELECT fullname, postcode, case_uuid, correspondent.type,
		case
