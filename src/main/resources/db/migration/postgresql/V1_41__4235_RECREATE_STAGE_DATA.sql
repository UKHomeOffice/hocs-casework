...
FROM casework.stage s
         ...
         LEFT JOIN LATERAL(
             SELECT json_build_object('correspondents', json_agg(json_build_object('fullname', fullname, 'postcode', postcode, 'type', "type", 'is_primary', is_primary))) as correspondents
             FROM (
                 SELECT fullname,
                        postcode,
                        case_uuid,
