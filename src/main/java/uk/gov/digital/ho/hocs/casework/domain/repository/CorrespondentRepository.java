...
public interface CorrespondentRepository extends CrudRepository<Correspondent, Long> {
    ...
    @Query(value = "SELECT c.* FROM correspondent c JOIN case_data cd ON c.case_uuid = cd.uuid WHERE c.case_uuid = ?1 AND NOT c.deleted AND NOT cd.deleted", nativeQuery = true)
    ...
    @Query(value = "SELECT c.* FROM correspondent c JOIN case_data cd on c.case_uuid = cd.uuid AND NOT cd.deleted", nativeQuery = true)
