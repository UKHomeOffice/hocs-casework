...
public class SummaryRepository {
    ...
    public List<Summary> findTeamsAndCaseCountByTeamUuid(Set<UUID> teamUuidSet) {
        ...
        query.unwrap(NativeQuery.class)
                .addScalar(TEAM_UUID_COLUMN_NAME, UUIDCharType.INSTANCE)
                .addScalar(COUNT_COLUMN_NAME, IntegerType.INSTANCE)
    ...
    public List<Summary> findUnallocatedCasesByTeam(Set<UUID> teamUuidSet) {
        Query query = entityManager.createNativeQuery("SELECT st.team_uuid as teamUuid, COUNT(*) FROM casework.stage st INNER JOIN casework.case_data cd ON st.case_uuid = cd.uuid WHERE st.team_uuid in ?1 AND st.user_uuid IS NULL AND NOT cd.deleted AND NOT cd.data @> CAST('{\"Unworkable\":\"True\"}' AS JSONB) GROUP BY st.team_uuid");
        query.setParameter(1, teamUuidSet);
    ...
    public List<Summary> findOverdueUserCasesInTeams(Set<UUID> teamUuidSet, String userUuid) {
        Query query = entityManager.createNativeQuery("SELECT st.team_uuid as teamUuid, COUNT(*) FROM casework.stage st INNER JOIN casework.case_data cd ON st.case_uuid = cd.uuid WHERE st.team_uuid in ?1 AND st.user_uuid = ?2 AND st.deadline < CURRENT_DATE AND NOT cd.deleted AND NOT cd.data @> CAST('{\"Unworkable\":\"True\"}' AS JSONB) GROUP BY st.team_uuid");
        query.setParameter(1, teamUuidSet);
    ...
    public List<Summary> findUserCasesInTeams(Set<UUID> teamUuidSet, String userUuid) {
        Query query = entityManager.createNativeQuery("SELECT CAST(st.team_uuid as varchar) as teamUuid, COUNT(*) FROM casework.stage st INNER JOIN casework.case_data cd ON st.case_uuid = cd.uuid WHERE st.team_uuid in ?1 AND st.user_uuid = ?2 AND NOT cd.deleted AND NOT cd.data @> CAST('{\"Unworkable\":\"True\"}' AS JSONB) GROUP BY st.team_uuid");
        query.setParameter(1, teamUuidSet);
        query.setParameter(2, userUuid);
