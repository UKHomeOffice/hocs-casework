...
import static uk.gov.digital.ho.hocs.casework.application.LogEvent.DOCUMENT_CLIENT_DELETE_DOCUMENT_SUCCESS;
import static uk.gov.digital.ho.hocs.casework.application.LogEvent.DOCUMENT_CLIENT_GET_DOCUMENTS_SUCCESS;
import static uk.gov.digital.ho.hocs.casework.application.LogEvent.DOCUMENT_CLIENT_GET_DOCUMENT_DTO_SUCCESS;
import static uk.gov.digital.ho.hocs.casework.application.LogEvent.DOCUMENT_CLIENT_GET_DOCUMENT_PDF_SUCCESS;
import static uk.gov.digital.ho.hocs.casework.application.LogEvent.DOCUMENT_CLIENT_GET_DOCUMENT_SUCCESS;
...
public class DocumentClient {
    ...
    public GetDocumentsResponse getDocuments(UUID caseUUID, String type) {
        String url = type == null
            ? String.format("/document/reference/%s", caseUUID)
            : String.format("/document/reference/%s/?type=%s", caseUUID, type);

        GetDocumentsResponse documents = restHelper.get(serviceBaseURL, url, GetDocumentsResponse.class);
        log.info("Got Documents {} for Case {}", documents.getDocumentDtos(), caseUUID,
    ...
    public DocumentDto getDocument(UUID documentUUID) {
        DocumentDto documentDto = restHelper.get(serviceBaseURL, String.format("/document/%s", documentUUID),
            DocumentDto.class);
        log.info("Got documentDto with id {}", documentDto.getUuid(),
            value(EVENT, DOCUMENT_CLIENT_GET_DOCUMENT_DTO_SUCCESS));
        return documentDto;
    }
    ...
    public S3Document getDocumentFile(UUID documentUUID) {
        ...
        log.info("Got document with length {} for id {}", document.getData().length, documentUUID,
            value(EVENT, DOCUMENT_CLIENT_GET_DOCUMENT_SUCCESS));
        return document;
    }
    ...
    public S3Document getDocumentPdf(UUID documentUUID) {
        ...
        log.info("Got document with length {} for id {}", document.getData().length, documentUUID,
            value(EVENT, DOCUMENT_CLIENT_GET_DOCUMENT_PDF_SUCCESS));
        return document;
    }
