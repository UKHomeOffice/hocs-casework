...
public class CaseDeadlineHelperTest {
    ...
    public void setUp() throws Exception {
        ...
        EXISTING_CASE =  new CaseData(
                1L,
                CASE_UUID,
                LocalDateTime.of(2020, Month.APRIL,1, 0,0),
                CASE_TYPE,
                CASE_REFERENCE,
                false,
                DATA_CLOB,
    ...
    public void testShouldOverrideTheStageDeadlinesToCaseDeadline() {
        ...
        LocalDate[] stageDeadlines = EXISTING_CASE.getActiveStages().stream().map(ActiveStage::getDeadline).toArray(LocalDate[]::new);
        LocalDate[] stageDeadlinesWarnings = EXISTING_CASE.getActiveStages().stream().map(ActiveStage::getDeadlineWarning).toArray(LocalDate[]::new);

        assertArrayEquals("That all stage deadlines are set correctly",expectedStageDeadlines,stageDeadlines);
        assertArrayEquals("That all stage deadline warnings are set correctly",expectedStageDeadlineWarnings,stageDeadlinesWarnings);
